#define analysis_data1_cxx
#include "analysis_data1.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void do_dz_steps( float fcsx, float fcsy, float fcsz, float dxdz, float dydz,
  ...
  //--- compute the isolation
   ...
   for ( auto it = hcal_ids.begin(); it != hcal_ids.end(); it++ ) {
      ...
      //-- only include here if length is non-zero.
      if ( length > 0 ) {
        //-- skip first HCAL hit if do_nm1 is true
         if ( do_nm1 && hcal_hit_index == 0 ) {
            if ( verbose ) printf("  HCAL :  id = %5d   L = %6.1f  E = %7.3f  pred E = %7.3f    diff2/reso2 = %7.3f  NOT INCLUDED\n", *it, length, energy, pred_e, diff2 / hcal_reso2 ) ;
         } else {
   ...
   if ( do_nm1 && energy_chi2_hcal > 0 && hcal_active_path_length.size() == 1 ) {
      printf("\n\n **************************************************\n\n") ;
      printf(" NM1 hcal energy chi2  non zero, but Nhcal hits = 1.  How???\n") ;
      printf("\n\n **************************************************\n\n") ;
      char b = getchar() ;
         if ( b == 'q' ) return ;
   }
...
void analysis_data1::load_fcs_rec_hit_energy_maps() {
...
void analysis_data1::Loop( bool arg_verbose, int max_events )
{
   if (fChain == 0) return;
   ...
   TH2F* h_chi2_nm1 = new TH2F( "h_chi2_nm1", "chi2", scan_npoints, -1*scan_xmax-0.5*scan_dx, scan_xmax+0.5*scan_dx, scan_npoints, -1*scan_ymax-0.5*scan_dx, scan_ymax+0.5*scan_dx ) ;
   TH2F* h_chi2_nm1_iso = new TH2F( "h_chi2_nm1_iso", "iso chi2", scan_npoints, -1*scan_xmax-0.5*scan_dx, scan_xmax+0.5*scan_dx, scan_npoints, -1*scan_ymax-0.5*scan_dx, scan_ymax+0.5*scan_dx ) ;
   TH2F* h_chi2_nm1_energy = new TH2F( "h_chi2_nm1_energy", "energy chi2", scan_npoints, -1*scan_xmax-0.5*scan_dx, scan_xmax+0.5*scan_dx, scan_npoints, -1*scan_ymax-0.5*scan_dx, scan_ymax+0.5*scan_dx ) ;
   ...
   tt_out -> Branch( "rc_nm1_chi2_energy_ecal", &rc_nm1_chi2_energy_ecal ) ;
   tt_out -> Branch( "rc_nm1_chi2_energy_hcal", &rc_nm1_chi2_energy_hcal ) ;
   tt_out -> Branch( "rc_nm1_chi2_energy", &rc_nm1_chi2_energy ) ;
   tt_out -> Branch( "rc_nm1_chi2_iso1_ecal", &rc_nm1_chi2_iso1_ecal ) ;
   tt_out -> Branch( "rc_nm1_chi2_iso1_hcal", &rc_nm1_chi2_iso1_hcal ) ;
   tt_out -> Branch( "rc_nm1_chi2_iso1", &rc_nm1_chi2_iso1 ) ;
   tt_out -> Branch( "rc_nm1_chi2_iso2_ecal", &rc_nm1_chi2_iso2_ecal ) ;
   tt_out -> Branch( "rc_nm1_chi2_iso2_hcal", &rc_nm1_chi2_iso2_hcal ) ;
   tt_out -> Branch( "rc_nm1_chi2_iso2", &rc_nm1_chi2_iso2 ) ;
   tt_out -> Branch( "rc_nm1_dx", &rc_nm1_dx ) ;
   tt_out -> Branch( "rc_nm1_dy", &rc_nm1_dy ) ;
   tt_out -> Branch( "rc_nm1_dx_gradient", &rc_nm1_dx_gradient ) ;
   tt_out -> Branch( "rc_nm1_dy_gradient", &rc_nm1_dy_gradient ) ;
   tt_out -> Branch( "rc_nm1_energy_ecal", &rc_nm1_energy_ecal ) ;
   tt_out -> Branch( "rc_nm1_energy_hcal", &rc_nm1_energy_hcal ) ;
   tt_out -> Branch( "rc_nm1_nhit_ecal", &rc_nm1_nhit_ecal ) ;
   tt_out -> Branch( "rc_nm1_nhit_hcal", &rc_nm1_nhit_hcal ) ;
   tt_out -> Branch( "rc_nm1_hcal_L1", &rc_nm1_hcal_L1 ) ;
   tt_out -> Branch( "rc_nm1_hcal_E1", &rc_nm1_hcal_E1 ) ;
   tt_out -> Branch( "rc_nm1_hcal_L2", &rc_nm1_hcal_L2 ) ;
   tt_out -> Branch( "rc_nm1_hcal_E2", &rc_nm1_hcal_E2 ) ;
   tt_out -> Branch( "rc_nm1_hcal_cell_id", &rc_nm1_hcal_cell_id ) ;
   tt_out -> Branch( "rc_nm1_hcal_cell_row", &rc_nm1_hcal_cell_row ) ;
   tt_out -> Branch( "rc_nm1_hcal_cell_col", &rc_nm1_hcal_cell_col ) ;
   tt_out -> Branch( "rc_nm1_hcal_cell_ns", &rc_nm1_hcal_cell_ns ) ;

   tt_out -> Branch( "rc_Pt", &rc_Pt ) ;
   tt_out -> Branch( "rc_Eta", &rc_Eta ) ;
   tt_out -> Branch( "rc_Phi", &rc_Phi ) ;
   tt_out -> Branch( "rc_mcPid", &rc_mcPid ) ;
   tt_out -> Branch( "rc_rci", &rc_rci ) ;
   tt_out -> Branch( "rc_ismu", &rc_ismu ) ;
   tt_out -> Branch( "rc_P", &rc_P ) ;
   tt_out -> Branch( "rc_Charge", &rc_Charge ) ;
   tt_out -> Branch( "rc_mcPt", &rc_mcPt ) ;
   tt_out -> Branch( "rc_mcEta", &rc_mcEta ) ;
   tt_out -> Branch( "rc_mcPhi", &rc_mcPhi ) ;
   tt_out -> Branch( "rc_mcdR", &rc_mcdR ) ;
   tt_out -> Branch( "rc_mcP", &rc_mcP ) ;
   tt_out -> Branch( "rc_mcCharge", &rc_mcCharge ) ;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      for ( int rci=0; rci<rcEta->size(); rci++ ) {
         ...
         if ( verbose ) {
            printf("\n\n\n +++++++++++++++++++++++++++++++++++++++++++\n") ;
            printf(" TRACK %3d:  Eta = %7.3f, Phi = %7.3f, Pt = %7.3f    n hits FTT = %d,  n hits FST = %d\n",
                rci, rcEta->at(rci), rcPhi->at(rci), rcPt->at(rci), rcNumFTT->at(rci), rcNumFST->at(rci)  ) ;
            printf(" tproj %3d: x,y,z = (%7.2f, %7.2f, %7.2f)\n",
                rci,  rcProjEcalx->at(rci), rcProjEcaly->at(rci), rcProjEcalz->at(rci) ) ;

            printf("\n") ;
         }

         if ( rcNumFST->at(rci) < 4 ) continue ;



         float ecal_trkproj_starx = rcProjEcalx->at(rci) ;
         float ecal_trkproj_stary = rcProjEcaly->at(rci) ;
         float ecal_trkproj_starz = rcProjEcalz->at(rci) ;
         ...
         float trk_star_px = rcProjEcalPx->at(rci) ;
         float trk_star_py = rcProjEcalPy->at(rci) ;
         float trk_star_pz = rcProjEcalPz->at(rci) ;
    ...
    //=================================  Section for chi2 calc with all FCS cells on the track ===========================================
         ...
         for ( int sxi = 0; sxi<scan_npoints; sxi++ ) {
            ...
            for ( int syi = 0; syi<scan_npoints; syi++ ) {
               ...
               do_dz_steps( scan_x, scan_y, ecal_trkproj_fcsz, trk_dxdz, trk_dydz,
                            ...
                            energy_chi2_ecal, energy_chi2_hcal, use_mc, do_nm1 ) ;
    ...
    //=================================  BEGIN Section for chi2 calc leaving out the first HCAL cell on the track ===========================================


         do_nm1 = true ;

         float nm1_iso1_chi2 ;
         float nm1_iso1_chi2_ecal, nm1_iso1_chi2_hcal ;
         float nm1_iso2_chi2 ;
         float nm1_iso2_chi2_ecal, nm1_iso2_chi2_hcal ;
         float nm1_energy_chi2 ;
         float nm1_energy_chi2_ecal, nm1_energy_chi2_hcal ;
         ...
         h_chi2_nm1 -> Reset() ;
         h_chi2_nm1_iso -> Reset() ;
         h_chi2_nm1_energy -> Reset() ;

         float nm1_best_chi2(99999999.) ;
         float nm1_min_x, nm1_min_y ;
         int nm1_necal(0), nm1_nhcal(0) ;
         int nm1_necal_enz(0), nm1_nhcal_enz(0) ;
         int nm1_hcal_cell_id(-1) ;
         int nm1_hcal_cell_row(-1) ;
         int nm1_hcal_cell_col(-1) ;
         int nm1_hcal_cell_ns(-2) ;

         float nm1_best_chi2_dx(0.) ;
         float nm1_best_chi2_dy(0.) ;
         float nm1_worst_chi2(0.) ;
         for ( int sxi = 0; sxi<scan_npoints; sxi++ ) {
            ...
            for ( int syi = 0; syi<scan_npoints; syi++ ) {
               ...
               do_dz_steps( scan_x, scan_y, ecal_trkproj_fcsz, trk_dxdz, trk_dydz,
                            nm1_iso1_chi2_ecal, nm1_iso1_chi2_hcal,
                            nm1_iso2_chi2_ecal, nm1_iso2_chi2_hcal,
                            nm1_energy_chi2_ecal, nm1_energy_chi2_hcal, use_mc, do_nm1 ) ;

               nm1_iso1_chi2 = nm1_iso1_chi2_ecal + nm1_iso1_chi2_hcal ;
               nm1_iso2_chi2 = nm1_iso2_chi2_ecal + nm1_iso2_chi2_hcal ;
               nm1_energy_chi2 = nm1_energy_chi2_ecal + nm1_energy_chi2_hcal ;

             //-- check for insufficient energy in either and set it for both if found.
               if ( nm1_iso1_chi2_ecal < 0 ) nm1_iso1_chi2 = nm1_iso1_chi2_ecal ;
               if ( nm1_iso1_chi2_hcal < 0 ) nm1_iso1_chi2 = nm1_iso1_chi2_hcal ;
               if ( nm1_iso2_chi2_ecal < 0 ) nm1_iso2_chi2 = nm1_iso2_chi2_ecal ;
               if ( nm1_iso2_chi2_hcal < 0 ) nm1_iso2_chi2 = nm1_iso2_chi2_hcal ;
               if ( nm1_energy_chi2_ecal < 0 ) nm1_energy_chi2 = nm1_energy_chi2_ecal ;
               if ( nm1_energy_chi2_hcal < 0 ) nm1_energy_chi2 = nm1_energy_chi2_hcal ;


               float nm1_chi2 = nm1_iso1_chi2 + nm1_energy_chi2 ;
               h_chi2_nm1 -> Fill( dx, dy, nm1_chi2 ) ;
               h_chi2_nm1_iso -> Fill( dx, dy, nm1_iso1_chi2 ) ;
               h_chi2_nm1_energy -> Fill( dx, dy, nm1_energy_chi2 ) ;

               if ( nm1_chi2 > 0 && nm1_chi2 < nm1_best_chi2 ) {
                  nm1_best_chi2 = nm1_chi2 ;
                  nm1_best_chi2_dx = dx ;
                  nm1_best_chi2_dy = dy ;
               }
               if ( nm1_chi2 > 0 && nm1_chi2 > nm1_worst_chi2 ) {
                  nm1_worst_chi2 = nm1_chi2 ;
               }
         ...
         if ( nm1_worst_chi2 <= 0 ) nm1_worst_chi2 = nm1_best_chi2 ;

       //--- allow for a closer point to be the best if the difference in chi2 is small.
         float nm1_min_global_chi2_at_closest_point = get_th2_min( h_chi2_nm1, nm1_min_x, nm1_min_y ) ;

         nm1_best_chi2_dx = nm1_min_x ;
         nm1_best_chi2_dy = nm1_min_y ;

         float nm1_best_chi2_dr = sqrt( nm1_best_chi2_dx*nm1_best_chi2_dx + nm1_best_chi2_dy*nm1_best_chi2_dy ) ;
         nm1_best_chi2 = nm1_min_global_chi2_at_closest_point ;



         if ( arg_verbose ) verbose = true ;

         if ( verbose ) printf("\n\n NM1 - Rerunning at best chi2 point.   dx = %5.2f  dy = %5.2f\n", best_chi2_dx, best_chi2_dy ) ;
         do_dz_steps( ecal_trkproj_fcsx + nm1_best_chi2_dx, ecal_trkproj_fcsy + nm1_best_chi2_dy, ecal_trkproj_fcsz, trk_dxdz, trk_dydz,
                      nm1_iso1_chi2_ecal, nm1_iso1_chi2_hcal,
                      nm1_iso2_chi2_ecal, nm1_iso2_chi2_hcal,
                      nm1_energy_chi2_ecal, nm1_energy_chi2_hcal, use_mc, do_nm1 ) ;
         ...
         nm1_nhcal = hcal_active_path_length.size() ;
         nm1_necal = ecal_active_path_length.size() ;
         int nm1_nfcs = nm1_nhcal + nm1_necal ;

        //-- get the track path length and energy for the first HCAL cell on the track.
         float nm1_hcal_L1(-10) ;
         float nm1_hcal_E1(-10) ;
         float nm1_hcal_L2(-10) ;
         float nm1_hcal_E2(-10) ;

         float nm1_energy_on_track_ecal(0.) ;
         float nm1_energy_on_track_hcal(0.) ;
         for ( auto it = ecal_active_path_length.begin(); it != ecal_active_path_length.end(); it++ ) {
            int hit_id = it->first ;
            if ( ecal_hit_energies->find( hit_id ) != ecal_hit_energies->end() ) nm1_energy_on_track_ecal += (*ecal_hit_energies)[ hit_id ] ;
         }
         int hcal_hit_index = 0 ;
         for ( auto it = hcal_active_path_length.begin(); it != hcal_active_path_length.end(); it++ ) {
            int hit_id = it->first ;
            if ( hcal_hit_energies->find( hit_id ) != hcal_hit_energies->end() ) {
               nm1_energy_on_track_hcal += (*hcal_hit_energies)[ hit_id ] ;
               if ( hcal_hit_index == 0 ) {
                  nm1_hcal_L1 = hcal_active_path_length[ hit_id ] ;
                  nm1_hcal_E1 = (*hcal_hit_energies)[ hit_id ] ;
                  nm1_hcal_cell_id = hit_id ;
                  hcal_id_to_row_col( hit_id, nm1_hcal_cell_row, nm1_hcal_cell_col ) ;
                  if ( ecal_trkproj_starx > 0 ) {
                     nm1_hcal_cell_ns = 0 ;
                  } else {
                     nm1_hcal_cell_ns = 1 ;
               ...
               if ( hcal_hit_index == 1 ) {
                  nm1_hcal_L2 = hcal_active_path_length[ hit_id ] ;
                  nm1_hcal_E2 = (*hcal_hit_energies)[ hit_id ] ;
               }
            }
            hcal_hit_index ++ ;
         }
       ...
       //-- check for insufficient energy in either and set it for both if found.
         ...
         if ( nm1_iso1_chi2_ecal == 0 && nm1_energy_chi2_ecal > 0 && nm1_energy_chi2_ecal < 1e7 ) nm1_iso1_chi2_ecal = 0.01 ;
         if ( nm1_iso1_chi2_hcal == 0 && nm1_energy_chi2_hcal > 0 && nm1_energy_chi2_hcal < 1e7 ) nm1_iso1_chi2_hcal = 0.01 ;
         if ( nm1_iso2_chi2_ecal == 0 && nm1_energy_chi2_ecal > 0 && nm1_energy_chi2_ecal < 1e7 ) nm1_iso2_chi2_ecal = 0.01 ;
         if ( nm1_iso2_chi2_hcal == 0 && nm1_energy_chi2_hcal > 0 && nm1_energy_chi2_hcal < 1e7 ) nm1_iso2_chi2_hcal = 0.01 ;

         if ( nm1_iso1_chi2 == 0 && nm1_energy_chi2 > 0 && nm1_energy_chi2 < 1e7 ) nm1_iso1_chi2 = 0.01 ; // for log10 plots
         if ( nm1_iso2_chi2 == 0 && nm1_energy_chi2 > 0 && nm1_energy_chi2 < 1e7 ) nm1_iso2_chi2 = 0.01 ; // for log10 plots

         float nm1_gradx, nm1_grady ;
         get_gradients( h_chi2_nm1, nm1_best_chi2_dx, nm1_best_chi2_dy, nm1_gradx, nm1_grady ) ;
         float nm1_best_dist(99.) ;

         if ( nm1_iso1_chi2 > 0 && nm1_iso1_chi2 < 1e6 && nm1_energy_chi2 > 0 && nm1_energy_chi2 < 1e6 ) {
            if ( nm1_gradx > nm1_grady ) {
               nm1_best_dist = nm1_best_chi2_dx ;
            } else {
               nm1_best_dist = nm1_best_chi2_dy ;
            }
         }
         ...
         if (  nm1_energy_chi2 > 0 ) {
            printf(" NM1 - event %6llu track %3d : necal = %d, nhcal = %d  Best chi2 = %9.2f ( %9.2f, %9.2f),  worst chi2 = %9.2f,   best/worst = %.5f,  dist=%7.3f",
               jentry, rci, nm1_necal, nm1_nhcal, nm1_best_chi2, nm1_energy_chi2, nm1_iso1_chi2, nm1_worst_chi2, nm1_best_chi2/nm1_worst_chi2, nm1_best_dist ) ;
            if ( mc_muon_match ) { printf(" ** muon") ; }
            printf("\n") ;
         }
    ...
    //=================================  END Section for chi2 calc leaving out the first HCAL cell on the track ===========================================
       ...
       //--- fill output branch values.
         ...
         rc_nm1_chi2_energy_ecal.push_back( nm1_energy_chi2_ecal ) ;
         rc_nm1_chi2_energy_hcal.push_back( nm1_energy_chi2_hcal ) ;
         rc_nm1_chi2_energy.push_back( nm1_energy_chi2 ) ;
         rc_nm1_chi2_iso1_ecal.push_back( nm1_iso1_chi2_ecal ) ;
         rc_nm1_chi2_iso1_hcal.push_back( nm1_iso1_chi2_hcal ) ;
         rc_nm1_chi2_iso1.push_back( nm1_iso1_chi2 ) ;
         rc_nm1_chi2_iso2_ecal.push_back( nm1_iso2_chi2_ecal ) ;
         rc_nm1_chi2_iso2_hcal.push_back( nm1_iso2_chi2_hcal ) ;
         rc_nm1_chi2_iso2.push_back( nm1_iso2_chi2 ) ;
         rc_nm1_nhit_ecal.push_back( nm1_necal ) ;
         rc_nm1_nhit_hcal.push_back( nm1_nhcal ) ;
         rc_nm1_dx.push_back( nm1_best_chi2_dx ) ;
         rc_nm1_dy.push_back( nm1_best_chi2_dy ) ;
         rc_nm1_dx_gradient.push_back( nm1_gradx ) ;
         rc_nm1_dy_gradient.push_back( nm1_grady ) ;
         rc_nm1_energy_ecal.push_back( nm1_energy_on_track_ecal ) ;
         rc_nm1_energy_hcal.push_back( nm1_energy_on_track_hcal ) ;
         rc_nm1_hcal_L1.push_back( nm1_hcal_L1 ) ;
         rc_nm1_hcal_E1.push_back( nm1_hcal_E1 ) ;
         rc_nm1_hcal_L2.push_back( nm1_hcal_L2 ) ;
         rc_nm1_hcal_E2.push_back( nm1_hcal_E2 ) ;

         rc_nm1_hcal_cell_id.push_back( nm1_hcal_cell_id ) ;
         rc_nm1_hcal_cell_row.push_back( nm1_hcal_cell_row ) ;
         rc_nm1_hcal_cell_col.push_back( nm1_hcal_cell_col ) ;
         rc_nm1_hcal_cell_ns.push_back( nm1_hcal_cell_ns ) ;
         ...
         rc_Phi.push_back( rcPhi->at(rci) ) ;
         ///////rc_Charge.push_back( rcCharge->at(rci) ) ;
         float track_theta = 2. * atan( exp( -1*(rcEta->at(rci)) ) ) ;
         float track_P = (rcPt->at(rci)) / sin(track_theta) ;
         rc_P.push_back(track_P) ;
     ...
     //     rc_mcPt.push_back( mcPt->at(mcind) ) ;
     //     rc_mcEta.push_back( mcEta->at(mcind) ) ;
     //     rc_mcPhi.push_back( mcPhi->at(mcind) ) ;
     //     rc_mcdR.push_back( dr ) ;
     //     rc_mcCharge.push_back( mcCharge->at(mcind) ) ;
     //     float mc_track_theta = 2. * atan( exp( -1*(mcEta->at(mcind)) ) ) ;
     //     float mc_track_P = (mcPt->at(mcind)) / sin(mc_track_theta) ;
     //     rc_mcP.push_back( mc_track_P ) ;
         } else {
            rc_mcPt.push_back( -1. ) ;
            rc_mcEta.push_back( -1. ) ;
            rc_mcPhi.push_back( -4. ) ;
            rc_mcdR.push_back( -0.1 ) ;
            rc_mcCharge.push_back( 0 ) ;
            rc_mcP.push_back( -1 ) ;
         }
...
//    rc_nm1_chi2_energy_ecal.clear() ;
//    rc_nm1_chi2_energy_hcal.clear() ;
//    rc_nm1_chi2_energy.clear() ;
//    rc_nm1_chi2_iso1_ecal.clear() ;
//    rc_nm1_chi2_iso1_hcal.clear() ;
//    rc_nm1_chi2_iso1.clear() ;
//    rc_nm1_chi2_iso2_ecal.clear() ;
//    rc_nm1_chi2_iso2_hcal.clear() ;
//    rc_nm1_chi2_iso2.clear() ;
//    rc_nm1_dx.clear() ;
//    rc_nm1_dy.clear() ;
//    rc_nm1_dx_gradient.clear() ;
//    rc_nm1_dy_gradient.clear() ;
//    rc_nm1_energy_ecal.clear() ;
//    rc_nm1_energy_hcal.clear() ;
//    rc_nm1_nhit_ecal.clear() ;
//    rc_nm1_nhit_hcal.clear() ;
//    rc_nm1_hcal_L1.clear() ;
//    rc_nm1_hcal_E1.clear() ;
//    rc_nm1_hcal_L2.clear() ;
//    rc_nm1_hcal_E2.clear() ;
//    rc_nm1_hcal_cell_id.clear() ;
//    rc_nm1_hcal_cell_row.clear() ;
//    rc_nm1_hcal_cell_col.clear() ;
//    rc_nm1_hcal_cell_ns.clear() ;
