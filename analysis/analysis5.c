...
void analysis5::load_fcs_mc_hit_energy_maps() {
...
void analysis5::load_fcs_rec_hit_energy_maps() {
...
void analysis5::Loop( bool arg_verbose, int max_events )
{
   if (fChain == 0) return;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      for ( int rci=0; rci<rcEta->size(); rci++ ) {
         ...
         if ( rcTrackId -> at(rci) >= 1 ) {
            ...
            if ( abs( mcPid->at(mcind) ) == 5 ) mc_muon_match = true ;
         }
       ...
       //--- allow for a closer point to be the best if the difference in chi2 is small.
         ...
         if ( iso_chi2 == 0 && energy_chi2 > 0 && energy_chi2 < 1e7 ) iso_chi2 = 0.01 ; // for log10 plots

         float gradx, grady ;
         get_gradients( h_chi2, best_chi2_dx, best_chi2_dy, gradx, grady ) ;
         float best_dist(99.) ;

         if ( iso_chi2 > 0 && iso_chi2 < 1e6 && energy_chi2 > 0 && energy_chi2 < 1e6 ) {
            if ( gradx > grady ) {
         ...
         if (  energy_chi2 > 0 ) {
            printf(" event %6llu track %3d : necal = %d, nhcal = %d  Best chi2 = %9.2f ( %9.2f, %9.2f),  worst chi2 = %9.2f,   best/worst = %.5f,  dist=%7.3f",
               jentry, rci, necal, nhcal, best_chi2, energy_chi2, iso_chi2, worst_chi2, best_chi2/worst_chi2, best_dist ) ;
            if ( mc_muon_match ) { printf(" ** muon") ; }
            printf("\n") ;
         }
         ...
         h_log10chi2_all -> Fill( log10(iso_chi2 + energy_chi2) ) ;
         h_log10chi2_energy_all -> Fill( log10(energy_chi2) ) ;
         h_log10chi2_iso_all -> Fill( log10(iso_chi2) ) ;
         h_log10chi2_energy_vs_iso_all -> Fill( log10(iso_chi2), log10(energy_chi2) ) ;

         if ( mc_muon_match ) {
            h_log10chi2_mu -> Fill( log10(iso_chi2 + energy_chi2) ) ;
            h_log10chi2_energy_mu -> Fill( log10(energy_chi2) ) ;
            h_log10chi2_iso_mu -> Fill( log10(iso_chi2) ) ;
            h_log10chi2_energy_vs_iso_mu -> Fill( log10(iso_chi2), log10(energy_chi2) ) ;
         } else {
            h_log10chi2_notmu -> Fill( log10(iso_chi2 + energy_chi2) ) ;
            h_log10chi2_energy_notmu -> Fill( log10(energy_chi2) ) ;
            h_log10chi2_iso_notmu -> Fill( log10(iso_chi2) ) ;
            h_log10chi2_energy_vs_iso_notmu -> Fill( log10(iso_chi2), log10(energy_chi2) ) ;
         }
