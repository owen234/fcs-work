...
#ifndef geom_code_check_h
#define geom_code_check_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   geom_code_check(TTree *tree=0);
   virtual ~geom_code_check();
   virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   virtual void     Loop( bool verbose=true, int max_events = -1 );
...
#ifdef geom_code_check_cxx
geom_code_check::geom_code_check(TTree *tree) : fChain(0) 
{
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      ...
      chain->Add("FULL-pv4-1kev-try4a_fwdtree.root/fwd");
...
geom_code_check::~geom_code_check()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t geom_code_check::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t geom_code_check::LoadTree(Long64_t entry)
{
...
void geom_code_check::Init(TTree *tree)
{
...
void geom_code_check::Show(Long64_t entry)
{
...
Int_t geom_code_check::Cut(Long64_t entry)
{
...
#endif // #ifdef geom_code_check_cxx
