...
void analysis4::load_fcs_mc_hit_energy_maps() {
...
void analysis4::load_fcs_rec_hit_energy_maps() {
...
//==================================================================================================================================================
void analysis4::Loop( bool use_mc, bool quiet )
{
...
////  if ( fcsN<=0 ) continue ;
      ...
      //if (fttN<=0) { printf("  %lld : fttN %d\n", jentry, fttN ) ; continue ; }
      if ( mcN<=0) { printf("  %lld : mcN %d\n", jentry, mcN ) ; continue ; }
      //if ( rcN<=0) { printf("  %lld : rcN %d\n", jentry, rcN ) ; continue ; }
      //if ( tprojN<=0) { printf("  %lld : tprojN %d\n", jentry, tprojN ) ; continue ; }
      if ( fcsN<=0 ) { printf("  %lld : fcsN %d\n", jentry, fcsN ) ; continue ; }
     ...
     //----------
      ...
      for ( int tpi=0; tpi<tprojN; tpi++ ) {
         ...
         if ( tprojIdD->at(tpi) != 5 ) continue ;

         float ecal_trkproj_starx = tprojX->at(tpi) ;
         float ecal_trkproj_stary = tprojY->at(tpi) ;
         float ecal_trkproj_starz = tprojZ->at(tpi) ;
         ...
         float trk_star_px = tprojPx->at(tpi) ;
         float trk_star_py = tprojPy->at(tpi) ;
         float trk_star_pz = tprojPz->at(tpi) ;
...
//       } // do_display ?
      ...
      if ( wait ) {
       //if (best_chi2 < 400 ) {
       //if ( necal_enz > 1 ) {
            printf("\n\n Done with event %llu out of %llu\n", jentry, nentries ) ;
            char answ = getchar() ;
            if ( answ == 'q' ) return ;
       //}
       //}
