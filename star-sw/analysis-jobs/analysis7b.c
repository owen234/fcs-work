#define analysis7b_cxx
#include "analysis7b.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void analysis7b::load_fcs_mc_hit_energy_maps() {
...
void analysis7b::load_fcs_rec_hit_energy_maps() {
...
void analysis7b::Loop( bool arg_verbose, int max_events )
{
   if (fChain == 0) return;
   ...
   TFile* tf_output = new TFile( Form("analysis-tree-job-%04d.root", job_index) , "RECREATE" ) ;

  //--- to save all input TTree branches in the output, use this.

   ///////TTree* tt_out = fChain -> CloneTree( 0 ) ;

  //--- to only save the new TTree branches in the output, use this.

   TTree* tt_out = new TTree( "fwd", "fwd" ) ;
...
/////    if ( nm1_energy_chi2_hcal > 0 && nm1_nhcal == 1 ) {
/////       printf("\n\n **************************************************\n\n") ;
/////       printf(" NM1 main loop, hcal energy chi2  non zero, but Nhcal hits = 1.  How???\n") ;
/////       printf("  necal = %d, nhcal = %d\n", nm1_necal, nm1_nhcal ) ;
/////       printf("\n\n **************************************************\n\n") ;
/////       char b = getchar() ;
/////          if ( b == 'q' ) return ;
/////    }
   ...
   saveHist( Form("hists-job-%04d.root", job_index),"h*" ) ;
