...
int StFwdTrackMaker::Init() {
    ...
    if (mGenTree) {
        ...
        //owen: additions
        mTree->Branch("fttMcpx",         &mTreeData. fttMcpx  );
        mTree->Branch("fttMcpy",         &mTreeData. fttMcpy  );
        mTree->Branch("fttMcpz",         &mTreeData. fttMcpz  );
        ...
        mTree->Branch("mcPid",      &mTreeData. mcPid ); // owen
        ...
        mTree->Branch("rcTrackIdT",  &mTreeData. rcTrackIdT );
        ...
        // owen: add FCS EMC and HCAL hits
        mTree->Branch("fcs_mc_ecalN", &mTreeData.fcs_mc_ecalN, "fcs_mc_ecalN/I" ) ;
        mTree->Branch("fcs_mc_ecalX", &mTreeData.fcs_mc_ecalX ) ;
        mTree->Branch("fcs_mc_ecalY", &mTreeData.fcs_mc_ecalY ) ;
        mTree->Branch("fcs_mc_ecalZ", &mTreeData.fcs_mc_ecalZ ) ;
        mTree->Branch("fcs_mc_ecalE", &mTreeData.fcs_mc_ecalE ) ;
        mTree->Branch("fcs_mc_ecalId", &mTreeData.fcs_mc_ecalId ) ;
        mTree->Branch("fcs_mc_ecalVid", &mTreeData.fcs_mc_ecalVid ) ;

        mTree->Branch("fcs_mc_hcalN", &mTreeData.fcs_mc_hcalN, "fcs_mc_hcalN/I" ) ;
        mTree->Branch("fcs_mc_hcalX", &mTreeData.fcs_mc_hcalX ) ;
        mTree->Branch("fcs_mc_hcalY", &mTreeData.fcs_mc_hcalY ) ;
        mTree->Branch("fcs_mc_hcalZ", &mTreeData.fcs_mc_hcalZ ) ;
        mTree->Branch("fcs_mc_hcalE", &mTreeData.fcs_mc_hcalE ) ;
        mTree->Branch("fcs_mc_hcalId", &mTreeData.fcs_mc_hcalId ) ;
        mTree->Branch("fcs_mc_hcalVid", &mTreeData.fcs_mc_hcalVid ) ;


        mTree->Branch("fcs_rec_ecalN", &mTreeData.fcs_rec_ecalN, "fcs_rec_ecalN/I" ) ;
        mTree->Branch("fcs_rec_ecalX", &mTreeData.fcs_rec_ecalX ) ;
        mTree->Branch("fcs_rec_ecalY", &mTreeData.fcs_rec_ecalY ) ;
        mTree->Branch("fcs_rec_ecalZ", &mTreeData.fcs_rec_ecalZ ) ;
        mTree->Branch("fcs_rec_ecalLX", &mTreeData.fcs_rec_ecalLX ) ;
        mTree->Branch("fcs_rec_ecalLY", &mTreeData.fcs_rec_ecalLY ) ;
        mTree->Branch("fcs_rec_ecalE", &mTreeData.fcs_rec_ecalE ) ;
        mTree->Branch("fcs_rec_ecalId", &mTreeData.fcs_rec_ecalId ) ;

        mTree->Branch("fcs_rec_hcalN", &mTreeData.fcs_rec_hcalN, "fcs_rec_hcalN/I" ) ;
        mTree->Branch("fcs_rec_hcalX", &mTreeData.fcs_rec_hcalX ) ;
        mTree->Branch("fcs_rec_hcalY", &mTreeData.fcs_rec_hcalY ) ;
        mTree->Branch("fcs_rec_hcalZ", &mTreeData.fcs_rec_hcalZ ) ;
        mTree->Branch("fcs_rec_hcalLX", &mTreeData.fcs_rec_hcalLX ) ;
        mTree->Branch("fcs_rec_hcalLY", &mTreeData.fcs_rec_hcalLY ) ;
        mTree->Branch("fcs_rec_hcalE", &mTreeData.fcs_rec_hcalE ) ;
        mTree->Branch("fcs_rec_hcalId", &mTreeData.fcs_rec_hcalId ) ;
...
void StFwdTrackMaker::loadFttHitsFromGEANT( FwdDataSource::McTrackMap_t &mcTrackMap, FwdDataSource::HitMap_t &hitMap, int count ){
    ...
    for (int i = 0; i < nstg; i++) {
        ...
        if (mGenTree && (unsigned)mTreeData.fttN < MAX_TREE_ELEMENTS) {
            ...
            //owen;
            mTreeData.fttMcpx.push_back( mcpx );
            mTreeData.fttMcpy.push_back( mcpy );
            mTreeData.fttMcpz.push_back( mcpz );
...
void StFwdTrackMaker::loadFcs( ) {
    ...
    //owen: add MC FCS ECAL and HCAL hits to TTree
    mTreeData.fcs_mc_ecalN = 0;
    mTreeData.fcs_mc_hcalN = 0;

    {
       St_g2t_emc_hit* hitTable_h = static_cast<St_g2t_emc_hit*>(GetDataSet("g2t_wca_hit"));
       if ( hitTable_h == 0x0 ) {
          LOG_INFO << " g2t_emc_hit table is empty." << endm ;
       } else {
          const int nHits = hitTable_h->GetNRows();
          LOG_INFO << Form(" g2t_emc_hit table has  %d hits.", nHits) << endm ;
          const g2t_emc_hit_st* hit = hitTable_h->GetTable();
          for (int i=0; i < nHits; i++) {
             if (!hit) {hit++; continue;}
             mTreeData.fcs_mc_ecalX.push_back( hit->x ) ;
             mTreeData.fcs_mc_ecalY.push_back( hit->y ) ;
             mTreeData.fcs_mc_ecalZ.push_back( hit->z ) ;
             mTreeData.fcs_mc_ecalE.push_back( hit->de ) ;
             mTreeData.fcs_mc_ecalId.push_back( hit->id ) ;
             mTreeData.fcs_mc_ecalVid.push_back( hit->volume_id ) ;
             mTreeData.fcs_mc_ecalN++ ;
             hit++ ;
          }
    ...
    {
       St_g2t_hca_hit* hitTable_h = static_cast<St_g2t_hca_hit*>(GetDataSet("g2t_hca_hit"));
       if ( hitTable_h == 0x0 ) {
          LOG_INFO << " g2t_hca_hit table is empty." << endm ;
       } else {
          const int nHits = hitTable_h->GetNRows();
          LOG_INFO << Form(" g2t_hca_hit table has  %d hits.", nHits) << endm ;
          const g2t_hca_hit_st* hit = hitTable_h->GetTable();
          for (int i=0; i < nHits; i++) {
             if (!hit) {hit++; continue;}
             mTreeData.fcs_mc_hcalX.push_back( hit->x ) ;
             mTreeData.fcs_mc_hcalY.push_back( hit->y ) ;
             mTreeData.fcs_mc_hcalZ.push_back( hit->z ) ;
             mTreeData.fcs_mc_hcalE.push_back( hit->de ) ;
             mTreeData.fcs_mc_hcalId.push_back( hit->id ) ;
             mTreeData.fcs_mc_hcalVid.push_back( hit->volume_id ) ;
             mTreeData.fcs_mc_hcalN++ ;
             hit++ ;
          }
    ...
    //owen: add rec FCS ECAL and HCAL hits to TTree
    mTreeData.fcs_rec_ecalN = 0;
    mTreeData.fcs_rec_hcalN = 0;

    {
       ...
       StEvent *stEvent = static_cast<StEvent *>(GetInputDS("StEvent"));
       if ( stEvent != 0x0 ) {
          StFcsCollection* fcsCollection = stEvent->fcsCollection() ;
          if ( fcsCollection != 0x0 ) {
             /////////// printf("\n === Begin printing fcsCollection\n") ;
             /////////// fcsCollection->print(3);
             /////////// printf(" === End printing fcsCollection\n\n") ;
             for ( int det = 0; det <4; det++ ) {
                StSPtrVecFcsHit&  hits = fcsCollection->hits(det);
                for ( unsigned int hi=0; hi<hits.size(); hi++ ) {
                   StFcsHit* hit = hits[hi];
                   float lx, ly ;
                   float sx, sy, sz ;
                   fcsDb -> getLocalXYinCell( hit, lx, ly ) ;
                   StThreeVectorD s3v = fcsDb -> getStarXYZ( hit->detectorId(), lx, ly ) ;
                   sx = s3v.x() ;
                   sy = s3v.y() ;
                   sz = s3v.z() ;
                   ...
                   if ( det < 2 ) {
                      mTreeData.fcs_rec_ecalX.push_back( sx ) ;
                      mTreeData.fcs_rec_ecalY.push_back( sy ) ;
                      mTreeData.fcs_rec_ecalZ.push_back( sz ) ;
                      mTreeData.fcs_rec_ecalLX.push_back( lx ) ;
                      mTreeData.fcs_rec_ecalLY.push_back( ly ) ;
                      mTreeData.fcs_rec_ecalE.push_back( hit->energy() ) ;
                      mTreeData.fcs_rec_ecalId.push_back( hit->id() ) ;
                      mTreeData.fcs_rec_ecalN++ ;
                   } else {
                      mTreeData.fcs_rec_hcalX.push_back( sx ) ;
                      mTreeData.fcs_rec_hcalY.push_back( sy ) ;
                      mTreeData.fcs_rec_hcalZ.push_back( sz ) ;
                      mTreeData.fcs_rec_hcalLX.push_back( lx ) ;
                      mTreeData.fcs_rec_hcalLY.push_back( ly ) ;
                      mTreeData.fcs_rec_hcalE.push_back( hit->energy() ) ;
                      mTreeData.fcs_rec_hcalId.push_back( hit->id() ) ;
                      mTreeData.fcs_rec_hcalN++ ;
                   }
                } // hi
             } // det
          } // fcs collection ?
...
StFwdTrack * StFwdTrackMaker::makeStFwdTrack( GenfitTrackResult &gtr, size_t indexTrack ){
    ...
    ////////// printf(" owen-debug: here 2c\n") ; fflush(stdout) ;

    //------owen: this seems to be where it crashes in MB events.
    ...
    for ( auto s : gtr.trackSeed ){
        ...
        ////////////   printf("  owen-debug: FTT hit %2d  cov row 1:  %g  %g  %g\n", hitcount, fh->_covmat(0,0), fh->_covmat(1,0), fh->_covmat(2,0) ) ; fflush(stdout) ;
        ////////////   printf("  owen-debug: FTT hit %2d  cov row 2:  %g  %g  %g\n", hitcount, fh->_covmat(0,1), fh->_covmat(1,1), fh->_covmat(2,1) ) ; fflush(stdout) ;
        ////////////   printf("  owen-debug: FTT hit %2d  cov row 3:  %g  %g  %g\n", hitcount, fh->_covmat(0,2), fh->_covmat(1,2), fh->_covmat(2,2) ) ; fflush(stdout) ;
        float cov[9];
    ...
    ////////////   printf(" owen-debug: here 2c 2\n") ; fflush(stdout) ;

    hitcount = 0 ;
    for ( auto s : gtr.fstSeed ){
        ////////////   printf(" owen-debug: here 2c 3, hit %d   hit pointer %p\n", hitcount, s ) ; fflush(stdout) ;
        if ( s == 0x0 ) {
           printf("\n\n *** StFwdTrackMaker::makeStFwdTrack:  hit pointer from gtr.fstSeed returned null.  Skipping!!!\n" ) ;
           continue ;
        }
        FwdHit * fh = static_cast<FwdHit*>( s );
        ////////////   printf("  owen-debug: FTS hit %2d  cov row 1:  %g  %g  %g\n", hitcount, fh->_covmat(0,0), fh->_covmat(1,0), fh->_covmat(2,0) ) ; fflush(stdout) ;
        ////////////   printf("  owen-debug: FTS hit %2d  cov row 2:  %g  %g  %g\n", hitcount, fh->_covmat(0,1), fh->_covmat(1,1), fh->_covmat(2,1) ) ; fflush(stdout) ;
        ////////////   printf("  owen-debug: FTS hit %2d  cov row 3:  %g  %g  %g\n", hitcount, fh->_covmat(0,2), fh->_covmat(1,2), fh->_covmat(2,2) ) ; fflush(stdout) ;
        float cov[9];
        cov[0] = fh->_covmat(0,0); cov[3] = fh->_covmat(1,0); cov[6] = fh->_covmat(2,0);
        cov[1] = fh->_covmat(0,1); cov[4] = fh->_covmat(1,1); cov[7] = fh->_covmat(2,1);
        cov[2] = fh->_covmat(0,2); cov[5] = fh->_covmat(1,2); cov[8] = fh->_covmat(2,2);
    ...
    vector<float> zPlanes = {
        ...
        375.0, // EPD
        //-- Owen: temporarily change to face of detector, not expected shower max z position.
        /////715.0, //ECAL
        /////807.0 // HCAL
        710.16, //ECAL
        782.63 // HCAL
...
void StFwdTrackMaker::FillTTree(){
    ...
    if (mGenTree) {
        ...
        for ( size_t i = 0; i < maxToSave; i++ ){
            ...
            mTreeData.rcTrackIdT.push_back( i );
...
void StFwdTrackMaker::Clear(const Option_t *opts) {
    ...
    if (mGenTree){
        ...
        mTreeData.fttMcpx.clear();
        mTreeData.fttMcpy.clear();
        mTreeData.fttMcpz.clear();
        ...
        mTreeData.mcPid.clear() ;
        mTreeData.mcVertexId.clear() ;

        mTreeData.fcsX.clear() ;
        mTreeData.fcsY.clear() ;
        mTreeData.fcsZ.clear() ;
        mTreeData.fcsE.clear() ;

        mTreeData.fcs_mc_ecalX.clear() ;
        mTreeData.fcs_mc_ecalY.clear() ;
        mTreeData.fcs_mc_ecalZ.clear() ;
        mTreeData.fcs_mc_ecalE.clear() ;
        mTreeData.fcs_mc_ecalId.clear() ;
        mTreeData.fcs_mc_ecalVid.clear() ;

        mTreeData.fcs_mc_hcalX.clear() ;
        mTreeData.fcs_mc_hcalY.clear() ;
        mTreeData.fcs_mc_hcalZ.clear() ;
        mTreeData.fcs_mc_hcalE.clear() ;
        mTreeData.fcs_mc_hcalId.clear() ;
        mTreeData.fcs_mc_hcalVid.clear() ;

        mTreeData.fcs_rec_ecalX.clear() ;
        mTreeData.fcs_rec_ecalY.clear() ;
        mTreeData.fcs_rec_ecalZ.clear() ;
        mTreeData.fcs_rec_ecalLX.clear() ;
        mTreeData.fcs_rec_ecalLY.clear() ;
        mTreeData.fcs_rec_ecalE.clear() ;
        mTreeData.fcs_rec_ecalId.clear() ;

        mTreeData.fcs_rec_hcalX.clear() ;
        mTreeData.fcs_rec_hcalY.clear() ;
        mTreeData.fcs_rec_hcalZ.clear() ;
        mTreeData.fcs_rec_hcalLX.clear() ;
        mTreeData.fcs_rec_hcalLY.clear() ;
        mTreeData.fcs_rec_hcalE.clear() ;
        mTreeData.fcs_rec_hcalId.clear() ;
